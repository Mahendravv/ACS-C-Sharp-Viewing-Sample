...
public class ImageStamp : PccHandler
{
    ...
    private string ImageToBase64String(String ImageStampId)
    {
        using (Image image = Image.FromFile(imageStampPath))
        {
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                sb1.AppendFormat("{{\"dataHash\": \"{0}\", \"dataUrl\": \"{1}\"}}", CommonCode.Encoder.GetHashString(sb.ToString()), sb.ToString());
                return sb1.ToString();
            }
        }
    }
    ...
    private String GetPath(String ImageStampId)
    {
        ...
        return PccConfig.ImageStampFolder + CommonCode.Encoder.DecodeURLString(ImageStampId);

        //String fullPath = PccConfig.ImageStampFolder + CommonCode.Encoder.DecodeURLString(ImageStampId);
